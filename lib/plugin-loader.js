...
async function searchNpmPlugins() {
    ...
    //await npmLoad({});
    const localNpms = await listNpms(path.resolve('./node_modules', '..'));
    //npm.config.set('global', true);
    const globalNpms = await listNpms(path.resolve(sysDir, '..'));
    ...
    for (const a of globalNpms) {
        ...
        a.requirePath = path.resolve(sysDir, a.requirePath);
        list.push(a);
    }
    return list;
}
