...
async function startMqttServer() {
    ...
    globals.PubSub.sub('/v1/admin/client_settings', (re)=>{
        ...
        if( topicPrefix == null ){
            topicPrefix = globals.admin.getMyID();
            /*let macs = globals.admin.getMACs(true);
            // Pick the first MAC address
            for( topicPrefix in macs)
                break;
            */
            if( topicPrefix == null ) // No proper topicPrefix found
                return ;
        }

        let mqttSettings = Object.assign({}, cs.mqtt);
        mqttSettings.topicPrefix = topicPrefix;
