...
module.exports.init = async function(_globals) {
    ...
    const [plugs] = await Promise.all([
        listPlugins(), genCryptKey(globals), startHttpServer()]);
...
async function startHttpServer() {
    const adminLocalStorage = PluginInterface.getLocalStoragePath(
        getStoragePath(), 'admin');
    const port = globals.cmd_opts.get('port') ||
        adminLocalStorage.getItem('PORT_NUM') || 8080;
    httpServer.start(port);

    // TODO:remover version '/v1'
    globals.PubSub.subscribe('/v1/admin/client_settings', (re)=>{
...
async function registerPluginInterface(
    ...
    const pi = new PluginInterface(
        globals, pluginName, role, modulePath, legacy, getStoragePath());
    const imports = {};
        ...
        try {
            if (procedure.length == 0) { // access for '/v1/' => plugin list
                ...
                for (let [prfx, plugin] of Object.entries(Plugins)) {
                    ...
                    if (args.option === 'true') {
                        prms.push(pi.setting._getSettingsSchema());
                        prmsPrfx.push(prfx);
                        prms.push(pi.setting.getSettings());
                        prmsPrfx.push(prfx);
                        ps[prfx].option = {
                            leaf: false,
                            // ,doc:{short:'Plugin'}
