...
async function registerPluginInterface(pobj, pluginName, role, modulePath, legacy) {
    const pi = new PluginInterface(globals, pluginName, role, modulePath, legacy, getStoragePath());
    const imports = {};
    ...
    if (typeof pobj.init === 'function') {
        ...
        if (role.includes(ROLE.SERVER)) {
            if (typeof pobj.onCall !== 'function') {
                const msg = `The '${pluginName}' plugin for role 'server' needs to export onCall(method, path, args).`;
                throw new Error(msg);
            }
		...
		try {
			if( procedure.length == 0 ){ // access for '/v1/' => plugin list
				...
				for( let prfx in Plugins ){
                    if (!Plugins[prfx].role.includes(ROLE.SERVER)) {
                        continue;
                    }
					const pi = Plugins[prfx].pi;
					const imports = Plugins[prfx].imports;
					ps[prfx] = {
